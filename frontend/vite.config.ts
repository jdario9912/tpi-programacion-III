import { defineConfig } from "vite";
import { resolve } from "node:path";
import { fileURLToPath } from "node:url";
import tailwindcss from "@tailwindcss/vite";

const root = fileURLToPath(new URL(".", import.meta.url));

export default defineConfig({
  build: {
    rollupOptions: {
      input: {
        //d:aplicaion/dist/
        index: resolve(root, "index.html"),
        login: resolve(root, "src/pages/auth/login.html"),
        registro: resolve(root, "src/pages/auth/registro.html"),
        adminHome: resolve(root, "src/pages/admin/home.html"),
        productos: resolve(root, "src/pages/admin/productos.html"),
        pedidos: resolve(root, "src/pages/admin/pedidos.html"),
        categorias: resolve(root, "src/pages/admin/categorias.html"),
        clientHome: resolve(root, "src/pages/client/home.html"),
        producto: resolve(root, "src/pages/client/producto.html"),
        carrito: resolve(root, "src/pages/client/carrito.html"),
        pedido: resolve(root, "src/pages/client/pedido.html"),
        pedidosCliente: resolve(root, "src/pages/client/pedidos.html"),
      },
    },
  },
  base: "./",
  plugins: [tailwindcss()],
});
